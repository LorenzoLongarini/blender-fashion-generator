...
2. **Prepare the Dataset Structure**:
   ...
   - After running the above commands, ensure the generated **Sparse** and **Dense** folders, along with the **database.db**, are correctly placed in the TTC dataset structure as follows:
...
3. **Proceed with Training Using Instant NGP or Nerfacto**:
   - Use the `--skip-colmap` flag in your training command to bypass feature extraction and mapping. Ensure the dataset directory is properly prepared with the required files.
